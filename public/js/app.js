  ...
  function getAllFileItems(){
    ...
    state.categories.forEach(cat=>{
      ...
      cat.subs.forEach(sub=>{
        sub.items.forEach(it=> arr.push(Object.assign({}, it, {catId:cat.id, catName:cat.name, subId:sub.id, subName:sub.name})));
      });
    });
    return arr;
  }
  ...
  function renderFileSubcategory(cat, sub){
    const itemsHtml = sub.items.length
      ? '<div class="grid-media">'+sub.items.map(it=>renderMediaCard(Object.assign({},it,{catId:cat.id, subId:sub.id, catName:cat.name}))).join('')+'</div>'
    ...
    return '<div class="sub-section"><div class="sub-head">'+
      ...
      '</div>'+itemsHtml+
      (editing? ('<input type="file" multiple style="display:none" id="file_'+sub.id+'" onchange="handleFiles(event,\''+cat.id+'\',\''+sub.id+'\')">') : '')+
      '</div>';
  }
  ...
  function renderEmbedCard(cat, sub, item){
    ...
    if(item.embedUrl){
      return '<div class="embed-card"><div class="embed-actions">'+removeBtn+'</div><iframe src="'+item.embedUrl+'" loading="lazy" allowfullscreen></iframe></div>';
    }
    const bg = PLATFORM_COLORS[item.platform] || '#7c5cfc';
    return '<div class="embed-card fallback" style="background:'+bg+'"><div class="embed-actions">'+removeBtn+'</div>'+
  ...
  function renderStatsBar(){
    ...
    const totalItems = allFiles.length + allSocial.length;
