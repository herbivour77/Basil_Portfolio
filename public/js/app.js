  ...
  function persistState(){
    ...
    fetch('/api/data', {
      ...
      console.error('Portfolio save failed:', resp.status, data);
      const message = data && (data.detail || data.error) ? (data.detail || data.error) : ('HTTP '+resp.status);
      setSaveIndicator('error');
      const el = document.getElementById('save-indicator');
      if(el) el.title = message;
    }).catch((err)=>{
      console.error('Portfolio save request failed:', err);
      setSaveIndicator('error');
      const el = document.getElementById('save-indicator');
      if(el) el.title = err && err.message ? err.message : 'Network error';
    });
  }
